...
- **Event-Log** `settings["ratingEvents"]`, append-only (`logRatingEvent`). Pro Eintrag: `ts, date` (lokaler Tag via `localDay`), `id, category, old, new, delta, crossedFifty` (`crossedFifty` ist Legacy, wird noch geschrieben, aber nirgends mehr ausgewertet). Rating wird zentral über `applyRating` gesetzt.
...
- **XP/Level** (`levelInfo` über Population): `totalXP = xpSources()` = Summe aller `effectiveRating` der Population **plus Gruppen-Bonus** → rückwirkend. `XP_PER_LEVEL = 700` (Konstante). `level = floor(totalXP / 700)`, `xpInLevel`, `progress`, `rankIndex = min(10, floor(level/10))`.
- **Gruppen-Bonus (zweite XP-Quelle, aktiv)** — `groupBonusXP()` = `GROUP_BONUS_PER_MEMBER` (20) je bewertetem Nicht-Repräsentanten einer Gruppe (`rating != null`, kein `isDefault`). Dockt in `xpSources` an. Er ist während einer Session konstant (Nicht-Reps werden nie in einer Session bewertet) und kürzt sich aus dem Session-Delta heraus (steckt in beiden Seiten via `sessionXP`).
- **Session-Delta (eingefrorenes id-Set)** — `startRating` friert `sessionPopIds = dashPopulation()`-ids ein. `snapshotXP` (Start) und `sessionXP()` (Ende, genutzt in `buildReport.gainedXP` und als Ziel der XP-Animation in `renderFinish`) summieren **`xpValue` über genau dieses Set** plus `groupBonusXP`. Start und Ende nutzen identische Mitgliedschaft + identisches Wertmaß → ein Mid-Session-Exclude verschiebt keine Seite, das Delta bleibt rein bewertungsbezogen. **`xpValue(t)` = `rating ?? prevRating ?? 0`** (nicht `effectiveRating`): eine re-uploadete Textur wiegt ihren alten Score, sodass das Ausschließen eines re-uploadeten Reps (`rating` null, `prevRating` bleibt) beidseitig gleich wiegt → Delta 0 statt negativ. `buildReport` überspringt ausgeschlossene Texturen in den Listen; deren Delta 0 passt damit zur Headline. Das **live Dashboard-Level** (`levelInfo(dashPopulation())` in `renderDashboard`) bleibt unverändert — nur die Session-Rechnung nutzt das eingefrorene Set.
- **Erfolge (Achievements)** — `ACHIEVEMENTS`-Registry: `{id,name,desc,cond(ctx),xp}`. `achievementCtx(pop,events)` liefert `decentCount/ratedCount/popLen/donePct/level/streak`. `checkAchievements` schaltet neu erfüllte frei, persistiert `settings["achievements"]` (ID-Liste) und zeigt je einen Toast (`showAchievement` → `nextAchievement`, `soundAchievement`). Geprüft in `renderDashboard` (Pop + Events liegen vor). Dashboard-Karte `achievementsCardHtml`. **Seeds haben `xp:0`** — ein echter Achievement-XP-Bonus müsste erst in `sessionXP` (beide Delta-Seiten) gefaltet werden, sonst bricht die Delta-Konsistenz wie beim Gruppen-Bonus.
- **Ränge** `RANK_NAMES` (Holz, Stein, Bronze, Eisen, Gold, Redstone, Lapis, Smaragd, Amethyst, Diamant für Level 0–99; Netherit ab 100), Farben in `RANK_COLORS`.
...
- Bewerten-Flow: `startRating` → `renderRate` → `commitRate` → bei leerer Queue `maybeFinish` → `finishSession` → `renderFinish` (baut Status-Hero `statusHeroHtml` + Listen, startet XP-Animation `snapshotXP` → `sessionXP()`).
